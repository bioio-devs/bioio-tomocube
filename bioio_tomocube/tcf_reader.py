...
class TCFReader(Reader):
    ...
    * ``"3DFL/CH0"``, ``"3DFL/CH1"``, … — 3-D fluorescence channels
    ...
    def _read_single_frame(
        ...
        scene_group_key = TCFReader._scene_group_key(scene)
        with fs.open(path, "rb") as fobj:
            with h5py.File(fobj, "r") as f:
                return TCFReader._read_frame(f, scene_group_key, channel_idx, frame_idx)
    ...
    def _build_xarray(self, delayed: bool) -> xr.DataArray:
        ...
        if delayed:
            frames = [
                da.from_delayed(
                    dask.delayed(TCFReader._read_single_frame)(
